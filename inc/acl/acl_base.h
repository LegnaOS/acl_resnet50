 ...
 * 调用aclrtFree接口释放内存
 *
 * @param size [IN]  以字节为单位的数据大小
 *
 * @retval 指向已创建实例的指针。如果内存不足则为null
 *
 ...
 *
 * @par 函数
 * 只有aclDataBuffer类型的数据被销毁。
 * 当acldatadatbuffer接口传入的数据的内存
 * 调用*来创建aclDataBuffer类型的数据必须由用户释放
 *
 * @param  dataBuffer [IN]  指针指向aclDataBuffer
 *
 ...
 * @li T旧数据需要用户释放，否则可能发生内存泄漏泄漏
 * 调用aclGetDataBufferAddr接口获取旧的数据地址
 * 调用aclrtFree接口释放内存
 *
 * @param data [IN]  指向新数据的指针
 * @li 需要由用户管理，
 * 调用aclrtMalloc接口申请内存
 * 调用aclrtFree接口释放内存
 *

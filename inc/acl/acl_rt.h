 ...
 * @brief 从异常信息中获取任务id
 *
 * @param info [IN]   异常信息指针
 *
 * @retval 异常信息中的任务id
 * @retval 0xFFFFFFFF 如果info为空
 */
 ...
 *
 * @param devPtr [输出]  指针指向设备上已分配内存的指针
 * @param size [输入]     分配内存大小
 * @param policy [输入]   内存分配策略
 *
 * @retval ACL_SUCCESS The function is successfully executed.
 * @retval OtherValues Failure
 *
